    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body::before {
            ...
            background: radial-gradient(ellipse at 20% 50%, rgba(108, 60, 225, 0.15) 0%, transparent 60%),
        ...
        .platform-badges { display: flex; flex-wrap: wrap; justify-content: center; gap: 5px; margin: 6px 0 2px; }
        .platform-badge { font-size: 9px; padding: 3px 10px; border-radius: 20px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.06); color: var(--text-muted); }
        ...
        .stats-bar { display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px; background: var(--bg-card); border-radius: var(--radius); padding: 10px 6px; margin-bottom: 12px; border: 1px solid rgba(255,255,255,0.06); }
        ...
        .tabs { display: flex; gap: 4px; background: var(--bg-card); border-radius: var(--radius); padding: 4px; margin-bottom: 12px; border: 1px solid rgba(255,255,255,0.06); }
        .tab { flex: 1; padding: 10px 6px; font-size: 11px; font-weight: 600; text-align: center; color: var(--text-muted); cursor: pointer; border-radius: 12px; transition: all 0.3s ease; }
        .tab i { margin-right: 4px; }
        .tab.active { background: linear-gradient(135deg, var(--primary), var(--secondary)); color: white; box-shadow: 0 4px 20px rgba(108, 60, 225, 0.4); }
        ...
        .card { background: var(--bg-card); border-radius: var(--radius); padding: 16px; border: 1px solid rgba(255,255,255,0.06); backdrop-filter: blur(10px); margin-bottom: 12px; }
        ...
        .input-group i { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: var(--text-muted); font-size: 14px; }
        input[type="text"], select { width: 100%; padding: 12px 12px 12px 38px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); background: rgba(0,0,0,0.3); color: var(--text); font-size: 13px; font-family: inherit; transition: all 0.3s ease; outline: none; }
        input[type="text"]:focus, select:focus { border-color: var(--primary); box-shadow: 0 0 20px rgba(108, 60, 225, 0.15); }
        input[type="text"]::placeholder { color: var(--text-muted); }
        select { padding-left: 12px; appearance: none; cursor: pointer; }
        select option { background: #1a1a2e; color: var(--text); }
        .btn-primary { width: 100%; padding: 14px; border: none; border-radius: 12px; background: linear-gradient(135deg, var(--primary), var(--secondary)); color: white; font-size: 15px; font-weight: 700; cursor: pointer; transition: all 0.3s ease; font-family: inherit; }
        ...
        .download-btn { display: block; width: 100%; padding: 12px; margin-top: 8px; border: none; border-radius: 10px; background: linear-gradient(135deg, var(--success), #00C853); color: #000; font-size: 14px; font-weight: 700; text-align: center; text-decoration: none; cursor: pointer; transition: all 0.3s ease; font-family: inherit; }
        ...
        .loader { width: 36px; height: 36px; border: 3px solid rgba(255,255,255,0.1); border-top: 3px solid var(--secondary); border-radius: 50%; animation: spin 0.8s linear infinite; margin: 0 auto 8px; }
        @keyframes spin { to { transform: rotate(360deg); } }
        .video-player { width: 100%; border-radius: 10px; margin-top: 8px; background: #000; position: relative; }
        .video-player video { width: 100%; border-radius: 10px; max-height: 350px; background: #000; }
        .video-player .close-btn { position: absolute; top: 12px; right: 12px; background: rgba(0,0,0,0.7); border: none; color: white; padding: 4px 10px; border-radius: 50%; cursor: pointer; font-size: 14px; z-index: 10; }
        .video-player .video-title { font-size: 12px; color: var(--text-muted); padding: 4px 8px; text-align: center; background: rgba(0,0,0,0.5); border-radius: 0 0 10px 10px; }
        ...
        .profile-card { background: rgba(255,255,255,0.03); border-radius: 12px; padding: 12px; margin-bottom: 10px; border: 1px solid rgba(255,255,255,0.05); transition: all 0.3s ease; }
        ...
        .video-item { display: flex; justify-content: space-between; align-items: center; padding: 6px 10px; background: rgba(0,0,0,0.2); border-radius: 8px; margin-top: 6px; font-size: 11px; gap: 8px; flex-wrap: wrap; transition: all 0.3s ease; }
        ...
        .video-item .actions a { color: var(--secondary); text-decoration: none; font-weight: 600; padding: 2px 8px; background: rgba(0, 212, 255, 0.1); border-radius: 6px; transition: all 0.2s ease; font-size: 10px; white-space: nowrap; display: inline-flex; align-items: center; gap: 3px; }
        ...
        .admin-login { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.9); z-index: 1000; display: none; justify-content: center; align-items: center; padding: 20px; }
        .admin-login.active { display: flex; }
        .admin-login .login-box { background: var(--bg-dark); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--radius); padding: 30px; max-width: 360px; width: 100%; }
        ...
        .footer { text-align: center; padding: 16px 0 8px; border-top: 1px solid rgba(255,255,255,0.05); margin-top: 8px; }
        ...
        .admin-access { position: fixed; bottom: 80px; right: 16px; z-index: 100; }
        .admin-access button { background: linear-gradient(135deg, var(--primary), var(--secondary)); border: none; color: white; padding: 10px 14px; border-radius: 30px; font-size: 12px; font-weight: 700; cursor: pointer; box-shadow: 0 4px 20px rgba(108, 60, 225, 0.4); transition: all 0.3s ease; font-family: inherit; }
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card { background: var(--bg-card); border-radius: var(--radius); padding: 20px; border: 1px solid rgba(255,255,255,0.06); backdrop-filter: blur(10px); margin-bottom: 20px; }
        .card-title { font-size: 16px; font-weight: 700; margin-bottom: 16px; color: var(--secondary); display: flex; align-items: center; gap: 10px; }
        .input-group { margin-bottom: 12px; }
        .input-group label { font-size: 12px; color: var(--text-muted); display: block; margin-bottom: 4px; font-weight: 600; }
        .input-group input, .input-group select { width: 100%; padding: 12px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.1); background: rgba(0,0,0,0.3); color: var(--text); font-size: 13px; font-family: inherit; outline: none; transition: border-color 0.3s; }
        ...
        .btn { width: 100%; padding: 14px; border: none; border-radius: 12px; font-size: 15px; font-weight: 700; cursor: pointer; transition: all 0.3s ease; font-family: inherit; color: white; }
        ...
        .back-link { display: block; text-align: center; margin-top: 20px; color: var(--secondary); text-decoration: none; font-weight: 600; font-size: 13px; transition: color 0.3s; }
        .back-link:hover { color: var(--primary); }
        .profile-list { margin-top: 12px; }
        .profile-item { display: flex; justify-content: space-between; align-items: center; padding: 10px 12px; background: rgba(255,255,255,0.03); border-radius: 8px; margin-bottom: 6px; }
        .profile-item .name { font-weight: 600; font-size: 13px; }
        .profile-item .count { font-size: 11px; color: var(--text-muted); }
        .profile-item .delete-btn { color: var(--danger); background: none; border: none; cursor: pointer; font-size: 14px; padding: 4px 8px; border-radius: 6px; transition: background 0.2s; }
        .profile-item .delete-btn:hover { background: rgba(255, 107, 107, 0.1); }
        .video-preview { font-size: 12px; color: var(--text-muted); padding: 4px 8px; background: rgba(0,0,0,0.2); border-radius: 4px; margin-top: 4px; display: flex; justify-content: space-between; align-items: center; }
        hr { border: 1px solid rgba(255,255,255,0.06); margin: 16px 0; }
        .stats-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-bottom: 16px; }
        .stat-box { background: rgba(255,255,255,0.03); border-radius: 10px; padding: 12px; text-align: center; }
        ...
        .upload-status { margin-top: 12px; padding: 12px; border-radius: 10px; background: rgba(0,0,0,0.2); display: none; }
        .upload-status.success { display: block; border: 1px solid var(--success); }
        .upload-status.error { display: block; border: 1px solid var(--danger); }
        .upload-status .progress-bar { width: 100%; height: 4px; background: rgba(255,255,255,0.1); border-radius: 2px; margin-top: 8px; overflow: hidden; }
        .upload-status .progress-bar .fill { height: 100%; background: var(--secondary); transition: width 0.3s ease; }
        @media (max-width: 420px) {
            .container { padding: 10px; }
...
<body>
    <div class="container">
        ...
        <!-- Bulk Upload -->
        <div class="card">
            <div class="card-title"><i class="fas fa-layer-group"></i> Bulk Upload Videos (Max {{ max_upload }})</div>
            <form id="bulkUploadForm" action="/admin/bulk-upload" method="POST" enctype="multipart/form-data">
                <div class="input-group">
                    <label>Select Player</label>
                    <select name="cricketer_key" required>
                        <option value="">-- Select Player --</option>
                ...
                <div class="input-group">
                    <label>Select Multiple Videos (Shift+Click or Ctrl+Click)</label>
                    <input type="file" name="video_files[]" accept="video/*" multiple required>
                </div>
                <div class="input-group">
                    <label>Optional: Custom Titles (one per line, in same order)</label>
                    <textarea name="video_titles" rows="3" placeholder="Video 1 title&#10;Video 2 title&#10;Video 3 title" style="width:100%; padding:12px; border-radius:10px; border:1px solid rgba(255,255,255,0.1); background:rgba(0,0,0,0.3); color:var(--text); font-size:13px; font-family:inherit; outline:none;"></textarea>
                </div>
                <button type="submit" class="btn btn-primary" id="bulkUploadBtn">
                    <i class="fas fa-cloud-upload-alt"></i> Upload All Videos
                </button>
            </form>
        ...
        document.getElementById('bulkUploadForm').addEventListener('submit', async function(e) {
            ...
            statusDiv.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span><i class="fas fa-spinner fa-spin"></i> Uploading videos...</span>
            ...
            try {
                ...
                if (data.success) {
                    let failedHtml = '';
                    if (data.failed > 0) {
                        failedHtml = `<br><span style="color: var(--danger); font-size: 12px;">❌ ${data.failed} failed: ${data.failed_list.map(f => f.filename).join(', ')}</span>`;
                    }
                    statusDiv.className = 'upload-status success';
                    statusDiv.innerHTML = `
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span><i class="fas fa-check-circle" style="color: var(--success);"></i> ✅ Upload Complete!</span>
                            <span>${data.uploaded} uploaded, ${data.failed} failed</span>
                        </div>
                        ${failedHtml}
                        <div style="margin-top: 8px; font-size: 12px; color: var(--text-muted);">
                            Total: ${data.total} videos processed
                        </div>
                        <button onclick="location.reload()" style="margin-top: 8px; padding: 6px 16px; background: var(--primary); border: none; border-radius: 6px; color: white; cursor: pointer;">
                    ...
                    statusDiv.innerHTML = `
                        <span style="color: var(--danger);"><i class="fas fa-exclamation-circle"></i> Error: ${data.error}</span>
                    `;
                }
            } catch (error) {
                statusDiv.className = 'upload-status error';
                statusDiv.innerHTML = `
                    <span style="color: var(--danger);"><i class="fas fa-exclamation-circle"></i> Network error: ${error.message}</span>
                `;
            } finally {
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> Upload All Videos';
            }
        });
        
        // Show selected file count
        document.querySelector('input[name="video_files[]"]').addEventListener('change', function() {
            const count = this.files.length;
            const label = this.closest('.input-group').querySelector('label');
            if (label) {
                label.textContent = `Selected ${count} video(s) (Max ${this.getAttribute('multiple') ? 'unlimited' : '1'})`;
            }
        });
    </script>
</body>
...
def admin_bulk_upload():
    try:
        ...
        video_titles_text = request.form.get('video_titles', '')
        video_titles = [t.strip() for t in video_titles_text.split('\n') if t.strip()]
        ...
        for i, video_file in enumerate(video_files):
            ...
            try:
                if i < len(video_titles) and video_titles[i]:
